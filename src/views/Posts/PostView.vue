...
import { usePost } from "../../service/post/index";
  ...
  uploader
    ...
    .then((files) => {
      if (files.length === 0) {
        ...
        postInfo.image = files.map((f) => f.fileUrl)[0];
      }
    })
    .catch((err) => {
      console.error(err);
...
const deletePost = async (id) => {
  const res = await usePost.remove(id);
  console.log(res);
...
const updatePost = async (el) => {
  id.value = "";
  id.value = el._id;
  updateInfo.text = el.text;
  updateInfo.title = el.title;
  updateInfo.short = el.short;
  showUpdateModal.value = true;
};

const updatePost2 = async () => {
  console.log(id.value);
  const res = await usePost.update(id.value, updateInfo);
  console.log(res);
...
const addPost = async () => {
  const res = await usePost.create(postInfo);
  console.log(res);
...
<template>
      ...
      >
        ...
        <div data-aos="fade-up" data-aos-delay="1000" class="overflow-x-auto">
          ...
          >
            ...
            >
              <tr>
                <th scope="col" class="px-4 py-3 min-w-[200px]">Title</th>
                <th scope="col" class="px-4 py-3">Text</th>
                <th scope="col" class="px-4 py-3">Shortly</th>
            ...
            <tbody>
              <tr class="border-b dark:border-gray-700" v-for="(el, ind) in store.LIST">
                ...
                <td class="px-4 py-3">{{ el.text }}</td>
                <td class="px-4 py-3">{{ el.short }}</td>
                <td class="px-4 py-3"><img :src="el.image" alt="" /></td>
                <td class="px-4 py-3">
                  <abbr title="Remove"
                    ><i
                      class="bx bx-trash text-2xl mr-6 text-red-600 hover:text-red-900 duration-300 hover:cursor-pointer"
                      @click="deletePost(el._id)"
                    ></i
                  ></abbr>
                  <abbr title="Edit"
                    ><i
    ...
    >
      ...
      <div class="mb-7">
        <div class="flex gap-7">
          ...
          <h1 class="w-full text-lg">Shortly:</h1>
        </div>
        <div class="flex gap-7">
          <input v-model="postInfo.title" type="text" class="w-full rounded-xl" />
          <input v-model="postInfo.short" type="text" class="w-full rounded-xl" />
        </div>
      </div>
      <div class="mb-7">
        <div class="flex gap-7">
      ...
      <div class="mb-7">
        <div class="flex gap-7">
          <h1 class="w-full text-lg">Text:</h1>
        </div>
        <Editor
          v-model="postInfo.text"
          api-key="rz0caj7sjqn6qrnu0synzbqkb0ykwk3rz2pdwxbpqfcnnxp6"
          :init="{
  ...
  <n-modal v-model:show="showUpdateModal">
    <n-card
      ...
      aria-modal="true"
    >
      <h1 class="text-3xl font-bold mb-9">Edit the post</h1>
      <div class="mb-7">
        <div class="flex gap-7">
          <h1 class="w-full text-lg">Title:</h1>
          <h1 class="w-full text-lg">Shortly:</h1>
        ...
        <div class="flex gap-7">
          ...
          <input v-model="updateInfo.short" type="text" class="w-full rounded-xl" />
        </div>
      </div>
      <div class="mb-7">
        <div class="flex gap-7">
