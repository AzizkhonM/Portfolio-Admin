...
const deleteSite = async (id) => {
  const res = await useSocials.remove(id);
  console.log(res);
...
const updateSocial = async () => {
  ...
  const res = await useSocials.update(id.value, updateInfo);
  console.log(res);

  toast.success("Ijtimoiy tarmoqdagi sahifa muvaffaqiyatli tahrirlandi!", {
    autoClose: 2500,
    pauseOnHover: true,
  });
  setTimeout(() => {
...
const addSocial = async () => {
  const res = await useSocials.create(socialsInfo);
  console.log(res);
...
<template>
  ...
  >
    List of pages in social media sites
  </h1>
  ...
  <n-modal v-model:show="showCreateModal">
    <n-card
      ...
      aria-modal="true"
    >
      <h1 class="text-3xl font-bold mb-9">Add new page</h1>
      <div class="mb-7">
        <div class="flex gap-7">
          <h1 class="w-full text-lg">Social media site:</h1>
          <h1 class="w-full text-lg">Username:</h1>
        </div>
        <div class="flex gap-7">
          <input v-model="socialsInfo.site" type="text" class="w-full rounded-xl" />
          <input v-model="socialsInfo.username" type="text" class="w-full rounded-xl" />
        </div>
      </div>
      <div class="mb-7">
        <div class="flex gap-7">
  ...
  <n-modal v-model:show="showUpdateModal">
    <n-card
      ...
      aria-modal="true"
    >
      <h1 class="text-3xl font-bold mb-9">Edit the page</h1>
      <div class="mb-7">
        <div class="flex gap-7">
          <h1 class="w-full text-lg">Social media site:</h1>
          <h1 class="w-full text-lg">Username:</h1>
        ...
        <div class="flex gap-7">
          <input v-model="updateInfo.site" type="text" class="w-full rounded-xl" />
          <input v-model="updateInfo.username" type="text" class="w-full rounded-xl" />
        </div>
      </div>
      <div class="mb-7">
        <div class="flex gap-7">
